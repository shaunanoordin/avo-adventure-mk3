...
import { PLAYER_ACTIONS, TILE_SIZE, EXPECTED_TIMESTEP, LAYERS, DIRECTIONS, ROTATIONS } from '@avo/constants'

const INVULNERABILITY_WINDOW = 3000
const MOVE_ACTION_CYCLE_DURATION = 200

export default class HeroV2 extends Entity {
  constructor (app, col = 0, row = 0) {
    ...
    this.animationHorizontalFacing = DIRECTIONS.EAST
  ...
  paint (layer = 0) {
    ...
    const animationSpriteSheet = app.assets.heroV2
    ...
    if (layer === LAYERS.ATOMS_LOWER) {
      ...
      const tgtX = Math.floor(this.x + camera.x + SPRITE_OFFSET_X)
      const tgtY = Math.floor(this.y + camera.y + SPRITE_OFFSET_Y)

      // Experiment: use canvas transforms
      c2d.save()
      c2d.translate(tgtX, tgtY)
      c2d.scale(2, 2)
      c2d.drawImage(animationSpriteSheet.img, srcX, srcY, sizeX, sizeY, -sizeX / 2, -sizeY, sizeX, sizeY)
      c2d.restore()
    }
  }
  ...
  set rotation (val) {
    ...
    // Flip the horizontal/vertical facing of the Paper Mario-type sprite ONLY
    // if the rotation CROSSES a cardinal direction.
  ...
  getAnimationSpriteColumn () {
    const v = this.animationVerticalFacing
    const h = this.animationHorizontalFacing

    if (v === DIRECTIONS.SOUTH && h === DIRECTIONS.EAST) { return 0 }
    else if (v === DIRECTIONS.NORTH && h === DIRECTIONS.EAST) { return 1 }
    else if (v === DIRECTIONS.NORTH && h === DIRECTIONS.WEST) { return 2 }
    else if (v === DIRECTIONS.SOUTH && h === DIRECTIONS.WEST) { return 3 }
